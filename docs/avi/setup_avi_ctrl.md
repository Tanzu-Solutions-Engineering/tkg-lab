...
In the screenshots below, it is assumed you are using the `Seperate Management network and VIP network` configuration.  However, in the instructions we will point out any distinctions if you choose those use this network setup.  Here are the configuration parameters in one of our contributor's (guess who) homelab.
