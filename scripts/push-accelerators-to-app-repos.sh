...
APP_URI_PREFIX="git@${GITLAB_HOST}:${APP_GROUP_NAME}/"
...
do
  pushd ${TKG_LAB_SCRIPTS}/../../workdir/${proj}
    ...
    if [[ -f ./k8s/git/pre-commit ]]; then
      mv ./k8s/git/pre-commit ./.git/hooks
    fi

    git add .
    git commit -m 'init'
    git remote add apprepo ${APP_URI_PREFIX}${proj}

    git push apprepo main
