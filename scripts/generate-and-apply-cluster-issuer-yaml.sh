...
then
  ...
  then
    ...
    kubectl create secret generic azure-dns-service-account \
        --from-file=credentials.json=keys/azure-dns-credentials.json \
        -n cert-manager -o yaml --dry-run=client | kubectl apply -f-
    ...
    yq e -i '.spec.acme.solvers[0].dns01.azureDNS.resourceGroupName = "externaldns' generated/$CLUSTER_NAME/contour/contour-cluster-issuer.yaml
